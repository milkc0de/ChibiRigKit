...
ローカル起動した画面で「完成品をdistに保存」を押すと、親の `dist/<キャラ名>/` と `dist/<キャラ名>.zip` に保存します。ZIPは複数のファイルをひとまとめにしたものです。
...
出力フォルダとZIPの内容は、プレイヤー・ライセンス・起動用4ファイルの合計6ファイルです。画像や設定JSONはHTMLに内蔵し、重複する `assets/` やJSONは同梱しません。録画モーションを含める場合もHTML内に保存します。
...
**これは、このPCの中だけで動く表示用サーバーです。インターネットには公開されず、同じWi-Fiの別の端末からもアクセスできません。起動・表示にインターネット接続は不要です。** Python 3が未導入なら、インストール時には接続が必要です。

OBSの「ブラウザ」ソースで「ローカルファイル」をオフにし、URLに `http://127.0.0.1:5510/` を指定します。起動したターミナルは開いたままにし、終了するときは `Ctrl+C` を押します。ポート5510が使用中の場合は、先に起動しているサーバーを停止してください。

この起動方法は完成品の表示用です。カメラ・音声追従は、追跡モデルを準備済みのChibiRigKitで `npm run player` を使用します。サーバーがOBSの録画・配信を開始することはありません。
...
カメラ・音声での追従、独自モーションの録画・保存・再生、ヨー／ピッチ／ロールの個別倍率、長い髪の物理演算を親プロジェクトへ統合しました。
...
Codexによる制作では入力画像・確認画像・作業指示をCodexへ渡します。カメラ／マイク追従は端末内の処理です。Codexのローカル操作はキャラ制作フォルダ内の書き込みと検証に限定し、追加のネットワーク・フォルダ外書き込み・無制限実行を自動許可しません。最終的なdist出力は親側の処理で行います。リモートapp-serverへの接続は暗号化されたwssを使います。
