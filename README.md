...
| ファイル | 内容 |
| --- | --- |
| `index.html` | 保存時の設定で開けるプレビュー |
| `assets/` | 描画に使う画像パーツ・準備済みの継ぎ目画像 |
| `rig.project.json` | キャラの構成と画像データ |
| `motion.json` | 動きの設定 |
| `head-poses.json` | 顔の9方向の配置（対応キャラのみ） |
| `background.json` | 選択した背景の設定・画像 |
| `capture.chibimotion.json` | 同梱を選んだ場合のみ、録画モーションの数値 |
| `README.txt` / `LICENSE.txt` | 開き方とツールのライセンス |
