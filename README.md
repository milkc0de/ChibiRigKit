...
キャラフォルダにある `index.html` をChromeで開くと、動きを見られます。このHTMLには描画用画像が入っています。
...
「完成品一式をダウンロード」で、ZIPファイルを保存します。ZIPは複数のファイルをひとまとめにしたものです。

1. ダウンロードした `ChibiRigKit-character.zip` を展開します。
...
ブラウザ内の保存は、そのブラウザのデータを消すと失われることがあります。残したい状態は「全体の動きをJSON保存」または「完成品一式をダウンロード」でファイルに保存してください。
...
- [元プロジェクトから反映した改善点](docs/V14_LESSONS.md)
