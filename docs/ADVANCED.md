...
5. Python がパーツ、`rig.project.json`、単独で開ける `index.html` を生成
...
具体的な対応表・パラメータ・継ぎ目補完の範囲は [docs/V14_LESSONS.md](V14_LESSONS.md) を参照してください。準備済みの補修／継ぎ目画像を扱えますが、LaMaモデルの自動取得・推論は含みません。
...
「首を支点に頭全体をランダムに動かす」で、顔・白目・黒目・閉じ目・髪・頭の飾りを共通の支点で回転させます。「ヨー・ピッチ・ロール」はヨー0〜30°・ピッチ0〜20°・ロール0〜15°（初期値12°・8°・6°）。顔配置の編集中は停止します。動きJSONの `settings.neck_sway` と `settings.neck_sway_degrees` に保存されます。支点は `rig.plan.json` の `neck_sway.pivot` で指定でき、省略時は顔の下端中央を使います。
