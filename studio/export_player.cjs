...
const {parsePlayer,zipHTML,zipTextFile}=require('./player_bundle.cjs');
...
async function writeExport(root,data,distRoot){
 ...
 const files={...launchers,'index.html':data.html,'LICENSE.txt':await fs.readFile(path.join(root,'LICENSE.txt'))};
