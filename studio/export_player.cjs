...
const {ensureTracking}=require('./tracking_assets.cjs');
...
async function writeExport(root,data,distRoot){
 ...
 const writes=[];for(const [relative,bytes] of Object.entries(files))writes.push([await safeTarget(dist,path.join(name,relative)),bytes]);writes.push([await safeTarget(dist,name+'.zip'),createZip(files)]);
 for(const [target,bytes] of writes){await fs.mkdir(path.dirname(target),{recursive:true});const temporary=target+'.'+require('node:crypto').randomUUID()+'.tmp';try{await fs.writeFile(temporary,bytes,{flag:'wx',mode:/\.(sh|command)$/.test(target)?0o755:0o644});await fs.rename(temporary,target)}finally{await fs.rm(temporary,{force:true})}}
