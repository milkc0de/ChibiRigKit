...
class PlayerOutputTests(unittest.TestCase):
    def test_html_and_zip_share_dist_without_touching_character_inputs(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            root=kit/'characters/hero';root.mkdir(parents=True);(root/'LICENSE.txt').write_text('TEST-LICENSE');(root/'MEDIA_NOTICE.txt').write_text('TEST-MEDIA');(root/'input').mkdir();(root/'input/normal.png').write_bytes(b'original')
            ...
            with zipfile.ZipFile(result['zip']) as z:
                self.assertIsNone(z.testzip());self.assertEqual(z.read('index.html').decode(),html);self.assertEqual(set(z.namelist()),{'index.html','LICENSE.txt','MEDIA_NOTICE.txt',*out.LAUNCHER_NAMES,*out.tracking_files(root)});self.assertEqual(z.read('LICENSE.txt'),b'TEST-LICENSE');self.assertEqual(z.read('MEDIA_NOTICE.txt'),b'TEST-MEDIA')
    ...
    def test_reexport_removes_previous_private_artifacts_but_keeps_unrelated_files(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            root=kit/'characters/hero';root.mkdir(parents=True);(root/'LICENSE.txt').write_text('TEST-LICENSE');(root/'MEDIA_NOTICE.txt').write_text('TEST-MEDIA')
            ...
            self.assertEqual({f.name for f in folder.iterdir()},{'index.html','LICENSE.txt','MEDIA_NOTICE.txt','my-note.txt','runtime','vendor','licenses',*out.LAUNCHER_NAMES})
