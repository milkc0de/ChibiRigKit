...
test('copied parent runtime starts without the old player directory or Electron dependencies',async()=>{
 ...
 try{
  for(const folder of ['studio','template/workspace/runtime'])await fs.cp(path.join(ROOT,folder),path.join(isolated,folder),{recursive:true});
  ...
  const origin=`http://127.0.0.1:${app.server.address().port}`,html=await(await fetch(origin)).text(),fixture=browserHarness(html);await fixture.context.rigReady;
